...
const bd=document.getElementById('bookingDate'); if(bd)bd.addEventListener('change',updateTimes);
const bf=document.getElementById('bookingForm'); if(bf)bf.addEventListener('submit',e=>{e.preventDefault();let bsel=document.getElementById('bookingService'),bt=document.getElementById('bookingTime');if(!bt.value)return;let service=services().find(s=>s.title===bsel.value);let b={id:uid(),clientFullName:clientFullName.value,clientEmail:clientEmail.value.trim().toLowerCase(),clientPhone:clientPhone.value,clientSocial:clientSocial.value,service:bsel.value,price:service?service.price:0,date:bookingDate.value,time:bt.value,comment:clientComment.value,status:'new',zoom:site().zoomLink,createdAt:new Date().toISOString()};let arr=bookings();arr.push(b);set('psy_bookings',arr);let cs=get('psy_clients',[]);if(!cs.some(c=>c.email===b.clientEmail)){cs.push({email:b.clientEmail,name:b.clientFullName,password:'123456'});set('psy_clients',cs)}bookingResult.style.display='block';bookingResult.innerHTML='<strong>Запис створено.</strong><br>'+esc(b.service)+'<br>'+b.date+' о '+b.time;e.target.reset();updateTimes();renderAll()});
const orf=document.getElementById('openReviewForm'); if(orf)orf.addEventListener('click',()=>document.getElementById('reviewFormCard').classList.toggle('active'));
