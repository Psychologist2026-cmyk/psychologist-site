...
(function(){
  ...
  function formatFooterContacts(){
    document.querySelectorAll('.footer-contacts a').forEach(row => {
      ...
      row.innerHTML = `<span class="social-icon-svg ${type}">${svg[type] || svg.site}</span><span class="contact-title-line">${title}</span><b class="contact-value-line">${value}</b>`;
      if(!href || href === '#') {
        if(type === 'telegram') row.href = '#';
  ...
  function restoreFixedCTA(){
    ...
    const telegram = site.telegramUrl || '#';
    box.innerHTML = `
      <a class="cta-book" href="services.html#booking">Записатись</a>
      <a class="cta-telegram" href="${telegram}" target="_blank">Telegram</a>
    `;
  }

  function markAdmin(){
    if(document.querySelector('.admin-layout,.admin-sidebar,#adminPanel,#offlineAddressSettingsMaster,#homeIdentitySettingsMaster')) {
      document.body.classList.add('admin-page');
    }
  }

  function removeBrokenOldFloaters(){
  ...
  function run(){
    markAdmin();
    removeBrokenOldFloaters();
    formatFooterContacts();
    restoreFixedCTA();
  }

  const old = window.renderAll;
  if(typeof old === 'function' && !window.__adminFooterCtaFix){
    window.__adminFooterCtaFix = true;
