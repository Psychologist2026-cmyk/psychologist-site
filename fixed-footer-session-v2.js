...
(function(){
  ...
  function contactRow(c){
    ...
    return `<a class="footer-contact-clean" href="${esc(link||'#')}"${target}>
      <span class="f2-icon ${type}">${icons[type]||icons.site}</span>
      <span class="f2-label">${esc(label)}</span>
      <b class="f2-value">${esc(value)}</b>
    </a>`;
  }
  ...
  function ensureFixedButtons(){
    document.querySelectorAll('.floating-actions,.float-actions,.sticky-actions,.sticky-cta,.quick-actions,.quick-contact,.fixed-cta,.fixed-buttons,.fixed-bottom-cta:not(.f2-fixed-cta),#fixedBottomCta:not(.f2-fixed-cta)').forEach(el=>el.remove());

    let box=document.querySelector('.f2-fixed-cta');
    if(!box){
    ...
    box.className='f2-fixed-cta';
    box.innerHTML=`<a class="f2-book" href="services.html#booking">Записатись</a><a class="f2-telegram" href="${esc(tg)}" target="_blank" rel="noopener">Telegram</a>`;
  }
  ...
  if(typeof old==='function' && !window.__fixedFooterSessionV2){
    window.__fixedFooterSessionV2=true;
