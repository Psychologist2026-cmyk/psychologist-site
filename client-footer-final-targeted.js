...
(function(){
  if(window.__clientFooterFinalTargeted) return;
  window.__clientFooterFinalTargeted = true;
  ...
  function getContacts(){
    ...
    // захист від порожніх значень, через які іконки “зникають”
    return arr.map(c => ({
      title: clean(c.title || c.name || ''),
      value: clean(c.value || c.url || c.phone || c.email || c.title || ''),
      link: clean(c.link || c.href || '#')
  ...
  function contactRow(c){
    ...
    let value = clean(c.value || c.title || label).replace(/[■▪●◆◼︎◾︎⬛︎□▫︎●•]/g,'');
    if(!value){
      if(type === 'telegram' || type === 'instagram') value = '@USERNAME';
      else if(type === 'email') value = 'psychologist@example.com';
      else if(type === 'phone') value = '+380 (67) 737 71 92';
      else value = label;
    }
    let link = clean(c.link || '#');
    if(type === 'email' && !link.startsWith('mailto:')) link = 'mailto:' + value;
    if(type === 'phone' && !link.startsWith('tel:')) link = 'tel:' + value.replace(/[^\d+]/g,'');
    const target = (link && link !== '#' && !link.startsWith('mailto:') && !link.startsWith('tel:')) ? ' target="_blank" rel="noopener"' : '';
    return `<a class="footer-contact-final" href="${esc(link || '#')}"${target}>
      <span class="footer-contact-final-icon ${type}">${icons[type] || icons.site}</span>
      <span class="footer-contact-final-label">${esc(label)}</span>
      <b class="footer-contact-final-value">${esc(value)}</b>
    </a>`;
  }
  ...
  function normalizeFooter(){
    ...
    const desired = getContacts().map(contactRow).join('');
    if(footer.dataset.finalFooter === 'yes' && footer.innerHTML === desired) return;

    internal = true;
    footer.id = 'footerContacts';
    footer.classList.add('footer-contacts');
    footer.innerHTML = desired;
    footer.dataset.finalFooter = 'yes';
    internal = false;
  }
  ...
  function ensureFixedCTA(){
    ...
    // прибираємо всі старі дублікати, окрім нашого
    document.querySelectorAll('.floating-actions,.float-actions,.sticky-actions,.sticky-cta,.quick-actions,.quick-contact,.fixed-cta,.fixed-buttons,.fixed-bottom-cta,#fixedBottomCta,.f2-fixed-cta,#guardFixedCta').forEach(el=>{
      if(el.id !== 'clientFooterFixedCta') el.remove();
    });

    let box = document.getElementById('clientFooterFixedCta');
    if(!box){
      box = document.createElement('div');
      box.id = 'clientFooterFixedCta';
    ...
    box.innerHTML = `<a class="book" href="services.html#booking">Записатись</a><a class="telegram" href="${esc(tg)}" target="_blank" rel="noopener">Telegram</a>`;
  ...
  function keepClientSession(){
    ...
    // В публічному меню замість "Увійти" показуємо "Мій кабінет"
    ...
    // Не дозволяємо випадковий logout на звичайних вкладках
  ...
  function autofillReviewName(){
    ...
    let client = null;
    try{
      client = (JSON.parse(localStorage.getItem('psy_clients')) || []).find(x => x.email === email);
    }catch(e){}
  ...
  if(typeof oldRender === 'function' && !window.__clientFooterFinalRenderHook){
    window.__clientFooterFinalRenderHook = true;
