...
(function(){
  ...
  function formatFooter(){
    document.querySelectorAll('.footer-contacts a').forEach(row=>{
      ...
      row.innerHTML = `<span class="social-icon-svg ${type}">${svg[type]||svg.site}</span><span class="contact-title-line">${title}</span><b class="contact-value-line">${esc(value)}</b>`;
  ...
  function markOldInjectedCards(){
    document.querySelectorAll('#homeIdentitySettingsMaster,#offlineAddressSettingsMaster').forEach(el=>{
      el.classList.add('admin-top-duplicate');
    });
  }

  function findAdminContentRoot(){
    return document.querySelector('.admin-content,.dashboard-content,.settings-content,.admin-main,main') || document.body;
  }

  function findTabTarget(name){
    const lower = name.toLowerCase();
    const possible = Array.from(document.querySelectorAll('section,div,main,article')).filter(el=>{
      const id=(el.id||'').toLowerCase();
      const cls=(el.className||'').toString().toLowerCase();
      const text=clean(el.querySelector('h1,h2,h3')?.textContent||'').toLowerCase();
      return id.includes(lower) || cls.includes(lower) || text.includes(name.toLowerCase());
    });
    return possible.find(el => el.offsetParent !== null) || findAdminContentRoot();
  }

  function injectHomeSettings(){
    if(!document.body.classList.contains('admin-page') && !document.querySelector('.admin-layout,.admin-sidebar,#adminPanel')) return;
    if(document.getElementById('cleanHomeSettingsSection')) return;

    const root = findTabTarget('головна');
    const site=read('psy_site',{});
    const wrap=document.createElement('section');
    wrap.className='admin-tab-injected-section';
    wrap.id='cleanHomeSettingsSection';
    wrap.innerHTML=`
      <h2>Головна сторінка</h2>
      <p>Редагування імені психолога, заголовка та короткого опису на головній сторінці.</p>
      <div class="admin-clean-form-grid">
        <label>Імʼя психолога
          <input id="cleanPsychologistName" value="${esc(site.psychologistName||'Анастасія Марчук')}">
        </label>
        <label>Заголовок на головній
          <input id="cleanHomeTitle" value="${esc(site.homeTitle||'Психотерапевт для важливих розмов')}">
        </label>
        <label class="wide">Короткий опис
          <textarea id="cleanHomeText">${esc(site.homeText||'')}</textarea>
        </label>
      </div>
      <div class="admin-save-row"><button class="btn primary" id="cleanSaveHomeBtn" type="button">Зберегти головну</button></div>
    `;
    root.prepend(wrap);

    wrap.querySelector('#cleanSaveHomeBtn').onclick=()=>{
      const next=read('psy_site',{});
      next.psychologistName=wrap.querySelector('#cleanPsychologistName').value.trim() || 'Анастасія Марчук';
      next.homeTitle=wrap.querySelector('#cleanHomeTitle').value.trim();
      next.homeText=wrap.querySelector('#cleanHomeText').value.trim();
      write('psy_site', next);
      alert('Головну сторінку збережено');
    };
  }

  function injectContactAddressSettings(){
    if(!document.body.classList.contains('admin-page') && !document.querySelector('.admin-layout,.admin-sidebar,#adminPanel')) return;
    if(document.getElementById('cleanContactAddressSection')) return;

    const root = findTabTarget('контакти');
    const wrap=document.createElement('section');
    wrap.className='admin-tab-injected-section';
    wrap.id='cleanContactAddressSection';
    wrap.innerHTML=`
      <h2>Адреси офлайн консультацій</h2>
      <p>Цей блок налаштовує адреси у вкладці “Контакти”. Можна додавати, редагувати, видаляти адреси та вказувати посилання Google Maps.</p>
      <div class="address-editor-list" id="cleanAddressEditorList"></div>
      <div class="admin-save-row">
        <button class="btn secondary" id="cleanAddAddressBtn" type="button">Додати адресу</button>
        <button class="btn primary" id="cleanSaveAddressBtn" type="button">Зберегти адреси</button>
      </div>
    `;
    root.appendChild(wrap);

    const render=()=>{
      const list=wrap.querySelector('#cleanAddressEditorList');
      const arr=getAddresses();
      list.innerHTML=arr.map((a,i)=>`
        <div class="address-editor-card" data-index="${i}">
          <label>Місто<input class="addr-city" value="${esc(a.city||'')}"></label>
          <label>Адреса<input class="addr-address" value="${esc(a.address||'')}"></label>
          <label class="addr-map-label">Google Maps<input class="addr-map" value="${esc(a.mapUrl||'')}" placeholder="Можна залишити пустим"></label>
          <button type="button" class="addr-edit-btn">Редагувати</button>
          <button type="button" class="addr-delete-btn">Видалити</button>
        </div>
      `).join('');
      list.querySelectorAll('.addr-delete-btn').forEach(btn=>{
        btn.onclick=()=>{
          const idx=Number(btn.closest('.address-editor-card').dataset.index);
      ...
      list.querySelectorAll('.addr-edit-btn').forEach(btn=>{
        btn.onclick=()=>btn.closest('.address-editor-card').querySelector('input').focus();
      });
    };

    wrap.querySelector('#cleanAddAddressBtn').onclick=()=>{
      const arr=getAddresses();
      arr.push({city:'',address:'',mapUrl:''});
      write('psy_offline_addresses', arr);
      render();
    };
    wrap.querySelector('#cleanSaveAddressBtn').onclick=()=>{
      const arr=Array.from(wrap.querySelectorAll('.address-editor-card')).map(card=>({
  ...
  function markAdmin(){
    if(document.querySelector('.admin-layout,.admin-sidebar,#adminPanel,#homeIdentitySettingsMaster,#offlineAddressSettingsMaster')) {
      document.body.classList.add('admin-page');
    }
  }

  function removeOldBrokenFloaters(){
  ...
  function run(){
    markAdmin();
    markOldInjectedCards();
    removeOldBrokenFloaters();
    formatFooter();
    restoreCTA();
    injectHomeSettings();
    injectContactAddressSettings();
  }

  const old=window.renderAll;
  if(typeof old==='function'&&!window.__adminStructureFooterFix){
    window.__adminStructureFooterFix=true;
